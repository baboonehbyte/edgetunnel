...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			if (!userID) {
				return new Response('Was ist deiner Vorname?', {
					status: 404,
					headers: {
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, env) {
	...
	if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
		...
		const 节点配置页 = `
			<br>
			<a href="javascript:void(0)" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?bauhaus','qrcode_0')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${proxyhost}${hostName}/${uuid}</a><br>
